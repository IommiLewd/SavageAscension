class barrierGenerator extends Phaser.Sprite {
    ...
    _addBarriers() {
        for (var i = 0; i < this.numberOfBarriers; i++) {
            if(i % 2 === 0){
            this.barrier = this.game.add.tileSprite(this.xModifier, 350 + this.yModifier, 448 , 36, 'barrier2');
            } else {
            this.barrier = this.game.add.tileSprite(this.xModifier, 350 + this.yModifier, 320 , 36, 'barrier');
            }
            this.xModifier += 150;
            this.yModifier += 360;
    ...
    _barrierRandomize(barrier){
        this.barrierWidth = Math.random() * (1100 - 100) + 100;
